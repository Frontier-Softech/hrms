<template>
	<div class="flex flex-col gap-5 w-full">
		<div class="flex flex-row justify-between items-center">
			<div class="text-lg text-gray-800 font-bold">{{ __("Upcoming Restricted Holidays") }}</div>
			<div
				v-if="holidays?.data?.length"
				id="open-holiday-list-1"
		...
		<EmptyState :message="__('You have no upcoming restricted holidays')" v-else />
	</div>

	<ion-modal
		ref="modal"
		v-if="holidays?.data?.length"
		trigger="open-holiday-list-1"
		:initial-breakpoint="1"
		:breakpoints="[0, 1]"
	>
		<div class="bg-white w-full flex flex-col items-center justify-center pb-5">
			<div class="w-full pt-8 pb-5 border-b text-center">
				<span class="text-gray-900 font-bold text-lg">{{ __("Restricted Holiday List") }}</span>
			</div>
...
const holidays = createResource({
	url: "hrms.api.get_restricted_holidays_for_employee",
	params: {
